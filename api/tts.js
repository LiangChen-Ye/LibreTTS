...
try {
  if (req.method === "POST") {
    ...
    const chunked = body.chunked === true; // 新增：支持chunked参数
    
    if (chunked) {
      return await handleTTSChunked(res, text, voiceName, rate, pitch, outputFormat, download);
    } else {
    ...
    const chunked = query.c === "true"; // 新增：支持chunked参数
...
try {
  ...
  if (download) {
    ...
    res.setHeader("Content-Disposition", `attachment; filename="${voiceName}.${extension}"`);
  }
...
// 新增：Chunked Transfer Encoding 版本的 handleTTS
export async function handleTTSChunked(res, text, voiceName, rate, pitch, outputFormat, download) {
try {
  await refreshEndpoint();
  ...
  console.error("TTS Chunked Error:", error);
  
  if (!res.headersSent) {
    // 如果还没开始发送数据，可以发送错误响应
    return res.status(500).json({ error: error.message });
  } else {
    // 如果已经开始streaming，只能结束连接
    res.end();
  }
}
}
